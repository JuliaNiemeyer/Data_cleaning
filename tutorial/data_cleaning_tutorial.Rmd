...
```{r occs}
species <- "Blanchetia heterotricha"
...
dir.create("../data_ju")
write.csv(occs, 
          "../data_ju/raw_data_Blanchetia_heterotricha.csv", 
          row.names = FALSE)
```
...
#no meu caso só tem um nome: "Blanchetia heterotricha DC" 

```{r sp-accepted}
table(occs$taxonomicStatus)
```
#No meu caso todos os 500 são accepted
...
##PERGUNTA: E NO CASO DE NÃO SER PLANTA??????
...
# Essa parte aqui não fez diferença nenhuma pra mim. Não sei se é pq não tem mais de um nome, mas não tem diferença entre occs e occs.new.tax
...
write.csv(occs.new.tax, 
          "../data/data_taxonomy_check_Blanchetia_heterotricha.csv", 
          row.names = FALSE)
```
...
write.csv(occs.new.geo2, 
          "../data_ju/data_coordinate_check_Blanchetia_heterotricha.csv", 
          row.names = FALSE)
```
